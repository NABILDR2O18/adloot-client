import { X, AlertTriangle, ImageIcon, InfinityIcon } from "lucide-react";
...
import { ILootablyOffer, PromotionConfig } from "@/pages/Offerwall";
import api from "@/lib/axios";

interface LootablyOfferPreviewModalProps {
  isOpen: boolean;
  onClose: () => void;
  offer?: ILootablyOffer | null; // We'll use static data for now
  app?: PromotionConfig;
  apiKey: string;
  userId: string;
}

export function LootablyOfferPreview({
  isOpen,
  onClose,
  offer,
  app,
  apiKey,
  userId,
}: LootablyOfferPreviewModalProps) {
        ...
        <section className="md:grid grid-cols-2 gap-6 p-4">
          <div className="space-y-4 order-2 md:order-1">
              ...
              {offer?.type === "singlestep" ? (
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                  <div className="flex items-center gap-3 max-w-[80%]">
                    <div className="min-w-8 h-8 rounded-full bg-white border border-gray-200 flex items-center justify-center text-sm font-medium text-gray-700">
                      1
                    </div>
                ...
                offer?.goals?.map((offerEvent, index) => (
                  <div
                    key={index}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-gray-700">
                        {offerEvent?.description}
                      </span>
                    </div>
                    <div className="font-medium text-gray-900">
                      {`+ ${offerEvent?.currencyReward} ${app?.currency_name_plural}`}
                    </div>
                  </div>
                ))
              )}
            ...
            <div className="pt-4 border-t border-gray-200">
              ...
              <div className="text-2xl font-bold text-gray-900 flex justify-start items-center">
                {offer?.type === "singlestep" ? (
                  offer?.currencyReward === "variable" ? (
                    <InfinityIcon />
                  ) : (
